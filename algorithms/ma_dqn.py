...
def train_ma_dqn(n_episodes, version):
    ...
    assert cfg['info'] != cfg['adv'] # use info or adv, not both!
    ...
    shutil.copyfile(madqn_cfg_file_path, os.path.join(log_path, madqn_cfg_file))
    
    dqn = DQN(n_states, n_actions, cfg)
    memory = deque(maxlen=cfg['episodes_without_training'] * env.spec.max_episode_steps)
    ...
    for episode in range(n_episodes + cfg['episodes_without_training'] + cfg['episodes_only_adv']):
        ...
        if episode > cfg['episodes_without_training']:
            cfg['adv_epsilon'] = max(cfg['adv_epsilon'] * cfg['adv_epsilon_decay'], 0.01)
